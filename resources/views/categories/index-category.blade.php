          	...
          	<div class="flex mb-4 items-center">
                <p class="w-full text-grey-darkest">{{$category->name}}</p>
                 <form action="{{ route('categories.destroy', $category) }}" method="post" >
                     @method('delete')
                     @csrf
                 <input class="p-2 pl-5 pr-5 bg-red-500 text-gray-100 text-lg rounded-lg focus:border-4 border-red-300" type="submit" value ="Delete">
                
</form >
                
                <a class="p-2 pl-5 pr-5 bg-green-500 text-gray-100 text-lg rounded-lg focus:border-4 border-green-300" href="{{ route('categories.edit', $category) }}">Edit</a>
